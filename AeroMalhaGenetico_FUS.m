function [ geom_malha,geom ] = AeroMalhaGenetico_FUS( geom, perfil_structure )
...
X_fus = repmat(transpose(c_fus * x),1,3);
Y_fus = repmat([0 .045 .09], length(X_fus),1);
Z_fus = zeros(size(Y_fus));
...
%     acum_fim = acum_panels(i+1);
    ...
    if i==1
        Xb = [X_fus,NaN(size(X_fus,1),1),X1];
        Yb = [Y_fus,NaN(size(X_fus,1),1),Y1];
        Zb = [Z_fus,NaN(size(X_fus,1),1),Z1];
%         Mi_w = Mi;
%         Ox_twist_w = Ox_twist;
