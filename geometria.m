...
c_root = aviao.c_root;              %[m] Corda na raiz
...
na = size(fb,1);                    %[-] Número de seções asa
...
ne = size(t_EH,1);                   %[-] Número de seções EH
...
% f_x_cg = aviao.f_x_cg;              %[-] Posição do CG em x/c_root
...
for i = 1:na
    aux = aux + ant*b_sec(i)/2 + ant*t(i)*b_sec(i)/2;
    ant = ant*t(i);
end
...
S_EH = 0;
while i<ne
    S_EH = S_EH + (ceh(i)+ceh(i+1))*fb_EH(i);
    i = i+1;
end
...
while i <= na+1
    x = x + c(i-1)/4*cosd(iw(i-1)) + tand(lw(i-1))*b/2*fb(i-1) - c(i)/4*cosd(iw(i)); %pura geometria, ver folha
    y = y + fb(i-1)*b/2;
    z = z - c(i-1)/4*sind(iw(i-1)) + tand(dw(i-1))*b/2*fb(i-1) + c(i)/4*sind(iw(i));
    pf(i,:) = [x y z];
    i = i+1;
end

%BA EH --------------------------------------------------------------------
pf(na+2,:) = [(c_root*cosd(i_w)+d_BF_BA_x) 0 (h_BA-c_root*sind(i_w)+d_BF_BA_z)]; %BA raiz da EH
i = i+1;
x = (c_root*cosd(i_w)+d_BF_BA_x);
y = 0;
z = (h_BA-c_root*sind(i_w)+d_BF_BA_z);
ch = c_root_EH;
ieh = i_EH;
while i <= nf
...
while (y<= ymax) && P<18
    y = y + dy;
    chord = c(1)/4*((t(1)-1)*(y/2)/(b*fb(1)) + 1); %corda nesse y
    xBA = c(1)/4*cosd(iw(1)) + tand(l(1))*(y/2) - chord/4*cosd(iw(1)+tw(1)*(y/(2*b*fb(1)))); %posicao do BA nesse y
...
pos_evx = ((origem(1) + folga):db:fim(1));
pos_evy = ((origem(2) + folga*inc_tb):(db*inc_tb):fim(2));
bracos_ev = pos_ev - xCG;
