...
geom.l = aviao.l_Q; 
geom.died = [0 0 0]; % A princípio é tudo zero
geom.twist = aviao.tw;
geom.offset_X = 0;
...
%% Incializa as variáveis do leme
% geom_ev.S = ((1 + aviao.t_ev) / 2.0 * aviao.cr_ev * aviao.b_ev);
% geom_ev.b = 2 * aviao.b_ev;
% geom_ev.AR = aviao.b_ev ^ 2 / geom_ev.S;
% geom_ev.i_wl = 0;
% geom_ev.fb = [];
% geom_ev.t = aviao.t_ev;
% zeros_ev = zeros(1,length(geom_ev.t));
% geom_ev.l = zeros_ev;
% geom_ev.died = zeros_ev;
% geom_ev.twist = zeros_ev;
% geom_ev.corda_raiz = aviao.cr_ev;
% geom_ev.perfis = [7, 7]; % NACA 0012
% geom_ev.dens = 100;
% geom_ev.MAC = geom.corda_raiz * 2 / 3 * (geom_ev.t ^ 2 + geom_ev.t + 1) / (geom_ev.t + 1);
% geom_ev.offset_X = 0;
% geom_ev.offset_Z = 0;
% geom_ev.inv_perfil = false;
% 
...
% [ geom_malha_ev(1), geom_ev ] = AeroMalhaGenetico( geom_ev, perfil_structure );
% % PlotaMalha(  geom_malha );

%% Calcula o cl_alpha da EV
% geom_painel_ev = AERO_PAINEL(geom_malha_ev);
% % condicoes
% run.sym = false;
% run.ground = false;
% 
% [inf_ev] = AERO_INF_VORING_V02(run, geom_painel_ev);
% [inf_ev_temp] = AERO_INF_VORING_V02(run, geom_painel_ev);
% inf_ev.B_H = inf_ev_temp.B;

% 5 graus
% run.alpha = 5;
% [inf_ev] = AERO_RHS(run, geom_painel_ev, inf_ev);
% coeff_alpha_ev = AERO_SECOMP_Genetico(run, geom_ev, geom_malha_ev, geom_painel_ev, inf_ev);

% restaura condicoes anteriores
run.alpha = 0;
run.sym = true;
...
aviao.CLev_alfa = 57.3 * coeff_alpha_ev(1).CL / 5;
...
aviao.CD_solo=coef_alpha0_solo(end).CD+CD_parasita;            %[-] CD total(induzido+parasita) do avião para alfa=0 em solo
...
aviao.alfa_trim_pure = -57.3*aviao.CM_zero/aviao.CM_alfa;
% aviao.nl_CL=NL_CL;
% aviao.nl_CM=NL_CM;
