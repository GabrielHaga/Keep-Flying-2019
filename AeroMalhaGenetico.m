function [ geom_malha,geom ] = AeroMalhaGenetico( geom, perfil_structure )
...
% Verdadeiro quando existe deflexão parcial
def_parcial = isfield(geom, 'def') && geom.def ~= 0 && geom.f_prof ~= 1;

% Deflexão de uma superfície totalmente atuável é equivalente a uma mudança
% de incidência na raiz
if isfield(geom, 'def') && geom.def ~= 0 && geom.f_prof == 1
    i_w = i_w - geom.def;
end
...
geom.panels_b_sec = panels_b_sec;
geom.panels_c = panels_c;
...
% caso com deflexão parcial
if def_parcial
    % índice em que deve começar a deflexão
    def_x = offsetX_inter_sec(end) + (1 - geom.f_prof) * corda_raiz * prod(t);
    def_idx = round((find(corda_raiz * x > def_x, 1) + find(x > 1 - geom.f_prof, 1)) / 2); 
end
...
%     acum_fim = acum_panels(i+1);
    ...
    % Faz com que cada seção da malha tenha um ponto com o 'x' da deflexão
    % (sim, eu sei tá uma gambiarra do krl)
    if def_parcial
        [~, cols] = size(X1);
        for jj = 1 : cols
            temp = X1(1, jj) + ...
                def_x * (X1(1 : def_idx, jj) - X1(1, jj))  / X1(def_idx, jj);
            X1(1 : def_idx - 1, jj) = temp(1 : end - 1);
            
            % FML
            r = (def_x - X1(end, jj)) / (X1(def_idx, jj) - X1(end, jj));
            X1(def_idx : end, jj) = X1(end, jj) + ...
                r * (X1(def_idx : end, jj) - X1(end, jj));
        end
    end
    
    % Posição em X no aerofólio
    foil_X = (X1 - min(X1)) ./ (max(X1) - min(X1));
    ...
    if def_parcial
        % Corrige as coordenadas em Z para contar os efeitos da mudança não
        % linear das coordenadas em X dos perfis das seções
%         Z_old = Z1_p_atual;
        Z1_p_atual = CorrigeZ(Z1_p_atual, foil_X, x);
        
        % Torce o BF para aplicar deflexão
        [end_X_def, end_Z_def] = ...
            RotacionaMatriz_Y(X1(def_idx : end, :) - def_x, Z1_p_atual(def_idx : end, :), - geom.def);
        end_X_def = end_X_def + def_x;
        
        X1(def_idx : end, :) = end_X_def;
        Z1_p_atual(def_idx : end, :) = end_Z_def;
    end
...
geom_malha = struct('X',Xb,'Y',Yb,'Z',Zb,'numelem',panels_b*panels_c,'cma_out',cma_out,'x',x2,'chord',chord,'b_sec', b_sec);

end

function Z = CorrigeZ(Z, Xf, xi)

[~, n] = size(Z);
for ii = 1 : n
    Z(:, ii) = interp1(xi, Z(:, ii), Xf(:, ii));
end
