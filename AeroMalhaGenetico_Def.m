function [ geom_malha,geom ] = AeroMalhaGenetico_Def( geom,perfil_structure )
% Malha feita para n seções 
%load('aero_dados_perfil_v0.mat')
%% número de painéis
...
teta=[linspace(0,acos(1-2*(1-fc)),ceil((panels_c+1)*(1-fc))-1) , linspace(acos(1-2*(1-fc)),pi,floor((panels_c+1)*fc)+2)];
teta(diff(teta)==0)=[];
x=0.5-0.5*cos(teta);
...
middle = NaN(comp_x,1);
% Acha a seção que termina a deflexão
ind_sec = find(cumsum(b_sec)-b*geom.fim_def>=0, 1 );
resto_def = b*geom.fim_def-acum_b(ind_sec);
...
for i = 1:num_sec
    ...
    bi = b_sec(i);
    if i==ind_sec
        p_def = ceil(resto_def/bi*(num_panels)); %Painéis da parte com deflexao  
        p_nao_def = num_panels+2-p_def;    %Painéis da parte sem deflexao
        y_ant = linspace(0,resto_def/bi,p_def);
        y_dep = linspace(resto_def/bi,1,p_nao_def);
        y0 = [y_ant,y_dep(2:end)];
   
    else
        y0 = linspace(0,1,num_panels+1);
...
%     acum_fim = acum_panels(i+1);
    ...
    Mx = repmat(x2,1,length(y0)); 
    a =  repmat(y0,comp_x,1); % Matriz auxiliar para vetorização
