function [inf_out]=AERO_INF_VORING_V02D (run,geom_painel)
...
% CPX=repmat((geom_painel.C1X+geom_painel.C2X)/2,1,num_total);
% CPY=repmat((geom_painel.C1Y+geom_painel.C2Y)/2,1,num_total);
% CPZ=repmat((geom_painel.C1Z+geom_painel.C2Z)/2,1,num_total);
% NX=repmat(geom_painel.NX,1,num_total);
% NY=repmat(geom_painel.NY,1,num_total);
% NZ=repmat(geom_painel.NZ,1,num_total);
...
C1X = (C1X+C2X)/2; C2X = C1X;

C3X = (C3X+C4X)/2; C4X = C3X;

is_TE=logical(repmat(geom_painel.is_TE',num_total,1));
FAR_DIST=1000*ones(sum(is_TE(:)),1);
