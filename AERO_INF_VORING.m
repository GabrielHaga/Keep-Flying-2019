function [inf_out] = AERO_INF_VORING (run,geom_painel)
...
%% Velocidades Induzidas por todos os segmentos de vórtice
...
v_ind_x=( v_ind_i(:,1:num_total)+sparse(idx_TE_x,idx_TE_y,v_ind_i_fwake(:,1) ) ).*NX;
v_ind_y=(v_ind_i(:,1+num_total:2*num_total)+sparse(idx_TE_x,idx_TE_y,v_ind_i_fwake(:,2)) ).*NY;
v_ind_z=(v_ind_i(:,1+2*num_total:3*num_total)+sparse(idx_TE_x,idx_TE_y,v_ind_i_fwake(:,3) )).*NZ;

a_inf=v_ind_x+v_ind_y+v_ind_z;

v_ind_x_w=(v_ind_w_i(:,1:num_total)+sparse(idx_TE_x,idx_TE_y,v_ind_w_i_fwake(:,1) )).*NX;
v_ind_y_w=(v_ind_w_i(:,1+num_total:2*num_total)+sparse(idx_TE_x,idx_TE_y,v_ind_w_i_fwake(:,2) )).*NY;
v_ind_z_w=(v_ind_w_i(:,1+2*num_total:3*num_total)+sparse(idx_TE_x,idx_TE_y,v_ind_w_i_fwake(:,3) )).*NZ;
