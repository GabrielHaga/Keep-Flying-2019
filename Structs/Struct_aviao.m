...
aviao.fb = [0, 0, 0];           %[m] Vetor com frações de envergadura
aviao.AR = 0;                   %[-] Razão de aspecto
aviao.i_w = 0;                  %[º] Incidência na raiz
aviao.t = [0, 0, 0, 0];         %[-] Vetor de taper em cada seção
aviao.l = [0, 0, 0, 0];         %[] Vetor enflechamento
aviao.tw = [0, 0, 0, 0];        %[°] Vetor de twist
aviao.d = [0, 0, 0, 0];         %[] Vetor de diedro em cada seção
aviao.perfil = [0, 0, 0, 0, 0]; %[-] Vetor de perfis em cada seção
...
aviao.t_EH = [0, 0];            %[-] Vetor de taper em cada seção da EH
...
aviao.c = [0, 0, 0, 0, 0];      %[m] Vetor das cordas de cada seção
aviao.S_EH = 0;
aviao.vetor_lv = [];
aviao.vetor_zmax = [];
aviao.b_sec = [];               %[m] Meia envergadura de cada seção
aviao.xCG = 0;
aviao.origem_cone = [];
...
aviao.CL_def = 0;               %[1/rad] Derivada do CL c/ relação a deflexão do profundor
aviao.CL_def_solo = 0;          %[1/rad] Derivada do CL c/ relação a deflexão do profundor em solo
aviao.CM_0 = 0;                 %[-] CM para alfa = 0
aviao.CM_0_solo = 0;            %[-] CM para alfa = 0 em solo
aviao.CM_alfa = 0;              %[1/rad] CM_alfa em relação ao CG
aviao.CM_alfa_solo = 0;         %[1/rad] CM_alfa em solo
aviao.CM_def = 0;               %[1/rad] Derivada do CM c/ relação a deflexão do profundor
aviao.CM_def_solo = 0;          %[1/rad] Derivada do CM c/ relação a deflexão do profundor em solo
...
aviao.CD_def = 0;               %[1/rad] Derivada do CD c/ relação a deflexão do profundor
aviao.CD_def_solo = 0;          %[1/rad] Derivada do CD c/ relação a deflexão do profundor em solo
...
aviao.xmotor = 0;               % Posição X do motor
aviao.zmotor = 0;
